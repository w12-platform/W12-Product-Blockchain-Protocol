...
library TokenListing {
    ...
    function hasCrowdsaleWithAddress(
        ...
        internal returns(uint)
    {
        ...
        if (!isTokenWhitelisted(whitelist, token)) {
            ...
            WhitelistedToken storage whitelisted = getToken(whitelist, token);

            whitelisted.name = name;
            whitelisted.symbol = symbol;
            whitelisted.decimals = decimals;
            whitelisted.feePercent = commissions[0];
            whitelisted.ethFeePercent = commissions[1];
            whitelisted.WTokenSaleFeePercent = commissions[2];
            whitelisted.trancheFeePercent = commissions[3];
            whitelisted.paymentMethods = paymentMethods;
            whitelisted.paymentMethodsPurchaseFee = paymentMethodsPurchaseFee;
        }
    ...
    function initializeCrowdsale(Whitelist storage whitelist, address token, address crowdsale) internal {
        ...
        Crowdsale storage notInitialized = getNotInitialisedCrowdsale(whitelist, token);

        notInitialized.crowdsale = crowdsale;
        notInitialized.feePercent = getToken(whitelist, token).feePercent;
        notInitialized.ethFeePercent = getToken(whitelist, token).ethFeePercent;
        notInitialized.WTokenSaleFeePercent = getToken(whitelist, token).WTokenSaleFeePercent;
        notInitialized.trancheFeePercent = getToken(whitelist, token).trancheFeePercent;
        notInitialized.owners = getToken(whitelist, token).owners;
        notInitialized.paymentMethods = getToken(whitelist, token).paymentMethods;
        notInitialized.paymentMethodsPurchaseFee = getToken(whitelist, token).paymentMethodsPurchaseFee;
